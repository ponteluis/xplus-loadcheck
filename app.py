...
def build_excel_file(resumen: pd.DataFrame, detalle: pd.DataFrame, total_malla: float, total_blastcenter: float, diferencia: float | None) -> bytes:
    ...
    resumen_control = pd.DataFrame([{
        "suma_total_kg_todas_las_hojas_guardadas": total_malla,
...
with tab_planilla:
    if uploaded_files:
            ...
            if st.button("Analizar planilla seleccionada", type="primary"):
                with st.spinner("Analizando imagen..."):
                    try:
                        ...
                        # Si es una planilla distinta a la anterior, se guardará como nueva.
                        st.session_state["hoja_actual_guardada_id"] = ""
        ...
        st.info("Sube una o varias fotos de planillas para comenzar. Las hojas ya guardadas se mantienen en el resumen aunque cargues otra imagen después.")

with tab_validacion:
    st.subheader("Tabla editable de verificación")
    st.caption("Edita solo los datos base. La tabla de control se recalcula automáticamente con cada cambio.")
    ...
    st.markdown("### Control automático de acumulados")
    st.caption("Esta tabla no se edita directamente. Se calcula desde kg por pozo y se usa para validar el acumulado del operador.")
    st.dataframe(
        tabla_calculada,
    ...
    with col_a:
        if st.button("Guardar hoja en resumen", type="primary"):
            ...
            st.success(f"Hoja guardada/actualizada en el resumen: {display_name}")
    with col_b:
...
with tab_resumen:
    ...
    if st.session_state["hojas_resumen"]:
        ...
        hoja_map = {f"{h['nombre_hoja']} | {h['archivo_original']}": h["hoja_id"] for h in st.session_state["hojas_resumen"]}
        ...
        st.info("Aún no hay hojas guardadas. Analiza una planilla y presiona “Guardar hoja en resumen”. La comparación con Blastcenter se hará con la suma de todas las hojas guardadas.")
