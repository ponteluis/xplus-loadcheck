...
def uploaded_file_info(uploaded_file):
    ...
    hoja_id = f"h_{file_hash}"
    return image_bytes, file_hash, file_key, hoja_id
...
def analyze_and_store_file(uploaded_file, index: int, model: str, make_current: bool = False):
    image_bytes, file_hash, file_key, hoja_id = uploaded_file_info(uploaded_file)
    mime_type = getattr(uploaded_file, "type", "image/jpeg") or "image/jpeg"
    display_name = st.session_state["nombres_hojas"].get(file_key, f"Hoja {index + 1}")

    data, model_used, usage = extract_with_gemini(image_bytes, mime_type, model)
    tabla = normalize_rows(data.get("rows", []))
    obs = data.get("observaciones_lectura", []) or []
    total, last_acc_op, diff_final_op, filas_diff, duplicates, tabla_calc = validate_table(tabla)
    ...
    if make_current:
        st.session_state["tabla_editable"] = tabla
        st.session_state["obs_lectura"] = obs
        ...
        st.session_state["nombre_hoja_actual"] = display_name
        st.session_state["hoja_actual_guardada_id"] = hoja_id
        st.session_state["editor_version"] = st.session_state.get("editor_version", 0) + 1
    ...
    return {
        ...
        "Total kg": total,
        "Alertas": filas_diff + len(duplicates),
        "Estado": "OK",
    }
...
with tab_planilla:
    if uploaded_files:
        st.subheader("Planillas cargadas")

        with st.expander("Renombrar hojas antes de analizar", expanded=False):
            for i, f in enumerate(uploaded_files):
                _, file_hash_tmp, file_key_tmp, _ = uploaded_file_info(f)
                if file_key_tmp not in st.session_state["nombres_hojas"]:
                    st.session_state["nombres_hojas"][file_key_tmp] = f"Hoja {i + 1}"
                st.session_state["nombres_hojas"][file_key_tmp] = st.text_input(
                    f"{f.name}",
                    value=st.session_state["nombres_hojas"][file_key_tmp],
                    key=f"nombre_batch_{reset_id}_{upload_version}_{file_hash_tmp}"
                )

        col_batch_a, col_batch_b = st.columns([0.35, 0.65])
        with col_batch_a:
            analizar_todas = st.button("Analizar todas las planillas cargadas", type="primary")
        with col_batch_b:
            st.caption("La app analizará cada imagen una por una internamente y las guardará en el Resumen de malla.")
        ...
        if analizar_todas:
            ...
            for i, f in enumerate(uploaded_files):
                status.write(f"Analizando {i + 1} de {len(uploaded_files)}: {f.name}")
                try:
                    result = analyze_and_store_file(
                        uploaded_file=f,
                        index=i,
                        model=model,
                        make_current=(i == len(uploaded_files) - 1)
                    )
            ...
            if results:
                st.success(f"Se analizaron y guardaron {len(results)} planilla(s) en el Resumen de malla.")
                st.dataframe(pd.DataFrame(results), use_container_width=True, hide_index=True)
            if errors:
                st.warning(f"No se pudieron analizar {len(errors)} archivo(s).")
                st.dataframe(pd.DataFrame(errors), use_container_width=True, hide_index=True)

        st.markdown("---")

        names = [f.name for f in uploaded_files]
        selected_name = st.selectbox("Revisar una planilla individual", names, key=f"select_file_{reset_id}_{upload_version}")
        selected_file = uploaded_files[names.index(selected_name)]

        image_bytes, file_hash, file_key, hoja_id = uploaded_file_info(selected_file)

        if file_key not in st.session_state["nombres_hojas"]:
            st.session_state["nombres_hojas"][file_key] = f"Hoja {names.index(selected_name) + 1}"
        ...
        with right:
            st.subheader("Lectura individual")
            st.info("Usa esta opción para revisar o corregir una hoja específica.")

            st.session_state["nombres_hojas"][file_key] = st.text_input(
                "Nombre de hoja para el resumen",
                value=st.session_state["nombres_hojas"][file_key],
                key=f"nombre_individual_{reset_id}_{upload_version}_{file_hash}"
            )

            if st.button("Analizar solo esta planilla"):
                with st.spinner("Analizando imagen..."):
                    try:
                        ...
                        st.success("Lectura completada y guardada en el resumen. Revisa la tabla si necesitas corregirla.")
                    except Exception as e:
        ...
        st.info("Sube una o varias fotos de planillas para comenzar. Puedes analizarlas todas con un solo botón.")
...
with tab_validacion:
    ...
    with col_a:
        if st.button("Guardar correcciones en resumen", type="primary"):
            ...
            st.success(f"Correcciones guardadas: {display_name}")
    with col_b:
